...
import { markVideoWatched, fetchCourseProgress } from "../../Redux/ProgressReducer/action";
...
export default function SinglePage() {
  ...
  const [showFinalQuizReminder, setShowFinalQuizReminder] = useState(false);
  const { id } = useParams();
    ...
    if (id && userId && token) {
      fetchCourseProgress(id, userId, token).then((progress) => {
        if (progress) {
          ...
          // Show reminder if course completed
          if (progress.isCompleted && !showFinalQuizReminder) {
            setShowFinalQuizReminder(true);
            toast({
              title: '🎉 Course Completed!',
              description: 'You\'ve finished all videos! Ready to take the final quiz and earn your certificate?',
              status: 'success',
              duration: 5,
              isClosable: true,
              position: 'top-right',
  ...
  }, [id, userId, token, showFinalQuizReminder, toast]);
